...
async function main() {
  ...
  // Настоящий reply на чьё-то сообщение. В форумах обычное сообщение в топике
  // тоже "отвечает" — на сам топик; это не ответ, его отсекаем.
  function getRealReplyToId(msg) {
    const rt = msg.replyTo;
    if (rt?.forumTopic && !rt.replyToTopId) return null;
    return getReplyToId(msg);
  }

  // Кому адресовано сообщение:
  //   'none'  — не ответ ни на что (общее предложение в чат)
  //   'me'    — ответ на сообщение юзербота
  //   'other' — ответ на чьё-то чужое сообщение (нас это не касается)
  async function getAddressee(msg, chatId, realReplyToId) {
    if (!realReplyToId) return 'none';
    const key = `${chatId}_${realReplyToId}`;
    if (store.ownPosts.has(key) || store.contexts.has(key)) return 'me';
    try {
  ...
  client.addEventHandler(async (event) => {
    ...
    // Новое предложение вз — отвечаем "сообщите", но только если оно адресовано нам:
    // общее предложение в чат или ответ на наше сообщение. Ответы другим людям игнорируем.
    if (!/(?:t\.me|telegram\.me)\//i.test(text)) return;
    const addressee = await getAddressee(msg, chatId, getRealReplyToId(msg));
    if (addressee === 'other') return;
    if (addressee === 'none' && config.data.answerGeneralOffers === false) return;

    const offer = await parseOffer(text, addressee === 'me');
    if (offer) {
