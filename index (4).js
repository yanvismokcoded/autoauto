...
async function main() {
  ...
  client.addEventHandler(async (event) => {
    ...
    console.log('event: chatId=', chatId, 'msgId=', msg.id, 'replyToId=', replyToId, 'text=', text, 'contextsHasReply=', replyToId ? store.contexts.has(replyToId) : null, 'contextsKeys=', [...store.contexts.keys()]);

    // Ответ "сообщите" на наше сообщение — тапаем
    if (replyToId) {
      const ctx = store.contexts.get(replyToId);
      if (ctx && text.toLowerCase().includes(config.data.confirmKeyword)) {
        ...
        store.contexts.delete(replyToId);
        return;
      }
    }
