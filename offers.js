...
const MAX_AGE_MS = 48 * HOUR; // предлагаем только тем, кто писал не позже 48 ч назад
...
const FALLBACK_MS = 60 * 60 * 1000; // согласие НЕ ответом на наше сообщение принимаем в течение часа
...
class Proposer {
  ...
  markPartner(authorId, username, status) {
    ...
    const upd = (map, key) => {
      const prev = map[key];
      map[key] = { status: (status === 'done' || (prev && prev.status === 'done')) ? 'done' : status, at: now };
    };
  ...
  isPartner(c, now) {
    const p = this.user.partners;
    const ok = (rec) => rec && (rec.status === 'done' || now - rec.at < AGREED_TTL_MS);
  ...
  async handleReply(msg, chatId) {
    ...
    try {
      ...
      if (rid) rec = offerMsgs[`${chatId}_${rid}`] || null;

      // человек ответил не «ответом» на наше сообщение, а просто в чат — принимаем
      // короткое согласие, если предложение было ему в этом чате не более часа назад
      if (!rec && !rid) {
        const off = this.user.offered[replierId];
        if (off && off.key && off.chatKey === String(chatId) &&
            Date.now() - off.at < FALLBACK_MS && t.length <= 60 && offerMsgs[off.key]) {
          rec = offerMsgs[off.key];
          fallback = true;
        }
      }
      if (!rec) return false;
      ...
      if (!agree) return !fallback; // отказ или болтовня — игнорируем
    ...
    try {
      switch (sub) {
        ...
        case 'off':
          ...
          cfg.nextAt = null;
          users.save();
          return ctx.reply('⏹ Новые предложения выключены. Тем, кому уже предложили, бот ещё ответит и потапает; ответы на чужие предложения работают как раньше.');
