...
// Генерирует читаемый одноразовый ключ вида "A1B2-C3D4"
...
function setupBot(botToken, userbot, tapper, config, store, log) {
  ...
  // ---------- Доступ: регистрация по секретному ключу ----------
  // Владелец бота (первый, кто написал /start) может выдавать ключи через /genkey.
  // Любой другой юзер должен ввести выданный ключ через /activate <ключ>,
  // прежде чем сможет пользоваться остальными командами.

  function isOwner(ctx) {
    return !!config.data.ownerId && String(ctx.from.id) === String(config.data.ownerId);
  }

  function isRegistered(ctx) {
    return isOwner(ctx) || !!store.data.users[String(ctx.from.id)];
  }

  const PUBLIC_COMMANDS = ['/start', '/activate'];

  bot.use((ctx, next) => {
    if (!ctx.from) return next(); // не личное сообщение от пользователя — пропускаем как есть
    ...
    return ctx.reply(
      '🔒 Доступ закрыт. Нужна регистрация: получите секретный ключ у владельца бота и введите\n' +
      '/activate <ключ>'
    );
  });

  bot.command('genkey', (ctx) => {
    if (!isOwner(ctx)) return ctx.reply('🔒 Эта команда только для владельца бота');
    const key = generateKey();
    store.data.pendingKeys[key] = { createdAt: Date.now() };
    store.save();
    ctx.reply(`🔑 Новый ключ для регистрации: \`${key}\`\nОтправьте его новому пользователю. Ключ одноразовый.`, { parse_mode: 'Markdown' });
  });

  bot.command('activate', (ctx) => {
    const key = parseArgs(ctx)[0];
    if (!key) return ctx.reply('Формат: /activate <ключ>');
    if (store.data.users[String(ctx.from.id)]) return ctx.reply('Вы уже зарегистрированы');

    const normalized = key.toUpperCase();
    if (!store.data.pendingKeys[normalized]) return ctx.reply('❌ Неверный или уже использованный ключ');

    delete store.data.pendingKeys[normalized];
    store.data.users[String(ctx.from.id)] = {
      key: normalized,
      registeredAt: Date.now(),
      username: ctx.from.username || null
    };
    store.save();
    ctx.reply('✅ Регистрация прошла успешно! Теперь вам доступны команды бота.');
  });
  ...
  bot.start((ctx) => {
    if (!config.data.ownerId) {
      ...
      config.save();
      ctx.reply('👑 Вы назначены владельцем бота (первый /start). Только вы можете выдавать ключи регистрации через /genkey.');
    }
    if (!isRegistered(ctx)) {
      return ctx.reply('🔒 Нужна регистрация. Получите секретный ключ у владельца бота и введите:\n/activate <ключ>');
    }
    ctx.reply(
      'VZ бот. Команды:\n' +
      (isOwner(ctx) ? '/genkey — выдать ключ регистрации новому пользователю\n' : '') +
      '/login — авторизация\n' +
      '/code <код> — ввод кода\n' +
      '/password <пароль> — 2FA\n' +
